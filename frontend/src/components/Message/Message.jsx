...
export const Message = ({ roomId, userName }) => {
  const { isConnected, socketResponse, sendData } = useSocket(roomId, userName);
  const [messageInput, setMessageInput] = useState("");
  const [messageList, setMessageList] = useState([]);

  const { responseData, error, loading } = useFetch("/message/" + roomId);
  ...
  return (
    <div className="message_root_div">
      <span className="room_name">Room: {roomId} </span>
